...
class ProjectPartiManager(QMainWindow):
        ...
        try:
            ...
            for result in cur.stored_results():
                for (prof,proy,horas) in result:
                    ...
                    self.ui.tabla.setItem(fila, 2, QTableWidgetItem(str(horas)))
                    fila += 1
            con.close()
            if fila == 0:
                QMessageBox.information(self, "Información", "No hay proyectos registrados")
        except Exception as e:
            print(e)
            QMessageBox.critical(self, "Error", "Error al conectar con la base de datos")
    ...
    def createPartiProy(self):
        if self.ui.spinBox.value() == 0:
            ...
            try:
                con = self.conection.conection()
                cur = con.cursor()
                cur.callproc("createPartiProy", (self.ui.Cprofesor.currentText().split("-")[0], self.ui.Cproyecto.currentText().split("-")[0], int(self.ui.spinBox.value())))
                con.commit()
                con.close()
                QMessageBox.information(self, "Información", "Proyecto registrado")
                self.getAllPartiProy()
            except Exception as e:
                print(e)
                QMessageBox.critical(self, "Error", "Error al conectar con la base de datos")
    ...
    def updatePartiProy(self):
        if self.ui.spinBox.value() == 0:
            ...
            try:
                con = self.conection.conection()
                cur = con.cursor()
                cur.callproc("updatePartiProy", (self.ui.Cprofesor.currentText().split("-")[0], self.ui.Cproyecto.currentText().split("-")[0], int(self.ui.spinBox.value()), int(self.a[0]), int(self.b[0])))
                con.commit()
                con.close()
    ...
    def deletePartiProy(self):
        try:
            con = self.conection.conection()
            cur = con.cursor()
            cur.callproc("deletePartiProy", (int(self.ui.Cprofesor.currentText().split("-")[0]), int(self.ui.Cproyecto.currentText().split("-")[0])))
            con.commit()
            con.close()
